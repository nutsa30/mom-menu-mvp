...
export default function RecipesClient({ dishes, locale, canRead, isLoggedIn }: Props) {
  ...
  return (
    <main className="min-h-screen bg-[#465940]">
      {/* Header */}
      <div className="border-b border-[#FDFBF0]/10">
        <div className="max-w-6xl mx-auto px-6 py-8">
          <h1 className="text-3xl font-black text-[#FDFBF0] mb-1">
            {t('რეცეპტები', 'Recipes')}
          </h1>
          <p className="text-[#FDFBF0]/50 text-sm">
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="relative mb-5">
          <svg className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#465940]/60" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
          <input
            value={search}
            onChange={(e) => { setSearch(e.target.value); setPage(1); }}
            placeholder={t('კერძის ძებნა...', 'Search recipes...')}
            className="w-full pl-10 pr-4 py-3 rounded-xl border border-[#465940]/20 focus:outline-none focus:border-[#465940] text-sm bg-[#FDFBF0] text-[#465940] shadow-sm"
          />
          {search && (
            <button
              onClick={() => { setSearch(''); setPage(1); }}
              className="absolute right-3.5 top-1/2 -translate-y-1/2 text-[#465940]/60 hover:text-[#465940]/80 transition"
            >
              ✕
            </button>
          )}
        ...
        <div className="mb-6">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[#FDFBF0] text-[#465940] text-sm font-bold border border-[#465940]/20 hover:border-[#465940] transition mb-3"
          >
            ...
            {(mealFilter !== 'ALL' || ageFilter !== 'ALL') && (
              <span className="w-2 h-2 rounded-full bg-[#465940] inline-block" />
            )}
          ...
          {/* Collapsible content */}
          {filtersOpen && (
            <div className="flex gap-4 sm:block bg-[#FDFBF0]/5 rounded-2xl p-4">
              {/* Meal type filter */}
              <div className="flex flex-col sm:flex-row gap-2 sm:flex-wrap sm:mb-3 flex-1">
                <p className="text-[10px] text-[#FDFBF0]/50 uppercase tracking-widest font-bold mb-1 sm:hidden">{t('ტიპი', 'Type')}</p>
                {MEAL_TYPES.map((m) => (
                  <button
                    key={m.key}
              ...
              <div className="flex flex-col sm:flex-row gap-2 sm:flex-wrap flex-1">
                <p className="text-[10px] text-[#FDFBF0]/50 uppercase tracking-widest font-bold mb-1 sm:hidden">{t('ასაკი', 'Age')}</p>
                {AGE_GROUPS.map((a) => (
                  <button
                    key={a.key}
                    ...
                    className={`px-3 py-2 rounded-full text-xs font-bold transition text-left sm:text-center ${
                      ageFilter === a.key
                        ? 'bg-[#465940] text-[#FDFBF0]'
                        : 'bg-[#FDFBF0] text-[#465940]/70 border border-[#465940]/20 hover:border-[#465940]/40'
                    }`}
                  >
                    {locale === 'ka' ? a.ka : a.en}
                  </button>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {paginated.map((dish) => (
              <button
                ...
                className="text-left rounded-2xl border border-[#FDFBF0]/20 overflow-hidden hover:border-[#FDFBF0]/40 hover:-translate-y-0.5 transition-all group"
              ...
              >
                ...
                <div className="p-4">
                  <h3 className="font-bold text-[#FDFBF0] text-sm leading-snug mb-1 line-clamp-2">{title(dish)}</h3>
                  {canRead && desc(dish) && (
                    <p className="text-xs text-[#FDFBF0]/65 line-clamp-2 mb-2">{desc(dish)}</p>
                  )}
                  {!canRead && (
                    <p className="text-xs text-[#FDFBF0]/50 blur-[3px] select-none mb-2 line-clamp-2">
                  ...
                  <div className="flex flex-wrap gap-1 mt-2">
                    {dish.ageGroups.map((ag) => {
                      ...
                      return (
                        <span key={ag} className="px-2 py-0.5 bg-[#FDFBF0]/15 text-[#FDFBF0] text-[10px] font-bold rounded-full">
                          {label ?? ag}
                        </span>
                      );
                    })}
                    {dish.calories && (
                      <span className="ml-auto text-[10px] text-[#FDFBF0]/50 font-medium">{dish.calories} kcal</span>
                    )}
                  </div>
                </div>
              </button>
          ...
          return (
            <div className="flex items-center justify-center gap-2 mt-10 flex-wrap">
              <button onClick={() => goToPage(page - 1)} disabled={page === 1}
                className="px-4 py-2 rounded-full text-sm font-bold border border-[#465940]/20 bg-[#FDFBF0] text-[#465940]/80 hover:border-[#465940] disabled:opacity-30 disabled:cursor-not-allowed transition">←</button>
              {items.map((p, i) =>
                p === null
                  ? <span key={`e${i}`} className="w-10 text-center text-[#FDFBF0]/60 text-sm select-none">…</span>
                  : <button key={p} onClick={() => goToPage(p)}
                      className={`w-10 h-10 rounded-full text-sm font-bold transition ${
                        p === page ? 'bg-[#465940] text-[#FDFBF0] shadow' : 'bg-[#FDFBF0] border border-[#465940]/20 text-[#465940]/80 hover:border-[#465940]'
                      }`}>{p}</button>
              )}
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-end sm:items-center justify-center p-4" onClick={() => setSelected(null)}>
          <div className="bg-[#FDFBF0] w-full max-w-lg rounded-2xl overflow-hidden max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="overflow-y-auto p-6 space-y-5">
              <div>
                <h2 className="text-xl font-black text-[#465940]">{title(selected)}</h2>
                <div className="flex flex-wrap gap-1.5 mt-2">
                  {selected.ageGroups.map((ag) => {
                    const label = locale === 'ka'
                      ? { FROM_6: '6თვ+', FROM_9: '9თვ+', FROM_12: '12თვ+', FROM_24: '24თვ+' }[ag]
                      : { FROM_6: '6mo+', FROM_9: '9mo+', FROM_12: '12mo+', FROM_24: '24mo+' }[ag];
                    return <span key={ag} className="px-2.5 py-0.5 bg-[#fff3ee] text-[#465940] text-xs font-bold rounded-full">{label ?? ag}</span>;
                  })}
                </div>
              </div>
              ...
              {canRead ? (
                <>
                  {desc(selected) && (
                    <div>
                      <p className="text-sm font-bold text-[#465940] mb-2">{t('მომზადების წესი', 'Preparation')}</p>
                      <ol className="space-y-1.5">
                        {parseSteps(desc(selected)).map((step, i) => (
                          <li key={i} className="flex gap-2.5 text-sm text-[#465940] leading-snug">
                            <span className="flex-shrink-0 w-5 h-5 rounded-full bg-[#fff3ee] text-[#465940] text-[11px] font-black flex items-center justify-center mt-0.5">{i + 1}</span>
                            <span>{step}</span>
                          </li>
                        ))}
                      </ol>
                  ...
                  {ingredients(selected).length > 0 && (
                    <div>
                      <p className="text-sm font-bold text-[#465940] mb-2">{t('ინგრედიენტები', 'Ingredients')}</p>
                      <ol className="space-y-1.5">
                  ...
                  {(() => {
                    const nutrients = [
                      { v: selected.calories,     label: t('კალორია','Calories'),       unit: 'kcal', color: 'bg-[#465940]/10 text-[#465940]' },
                      { v: selected.proteinGrams, label: t('ცილა','Protein'),            unit: 'g',    color: 'bg-[#465940]/10 text-[#465940]' },
                      { v: selected.carbsGrams,   label: t('ნახშირწყალი','Carbs'),       unit: 'g',    color: 'bg-[#465940]/10 text-[#465940]' },
                      { v: selected.fatGrams,     label: t('ცხიმი','Fat'),               unit: 'g',    color: 'bg-[#465940]/10 text-[#465940]' },
                      { v: selected.fiberGrams,   label: t('ბოჭკო','Fiber'),             unit: 'g',    color: 'bg-lime-50 text-lime-600' },
                      { v: selected.ironMg,       label: t('რკინა','Iron'),              unit: 'mg',   color: 'bg-[#465940] text-[#FDFBF0]' },
                      { v: selected.calciumMg,    label: t('კალციუმი','Calcium'),        unit: 'mg',   color: 'bg-[#465940]/10 text-[#465940]' },
                      { v: selected.zincMg,       label: t('თუთია','Zinc'),              unit: 'mg',   color: 'bg-[#465940] text-[#FDFBF0]' },
                      ...
                      { v: selected.sodiumMg,     label: t('ნატრიუმი','Sodium'),         unit: 'mg',   color: 'bg-[#465940]/5 text-[#465940]/80' },
                      ...
                      { v: selected.vitaminEmg,   label: t('E ვიტ.','Vit E'),            unit: 'mg',   color: 'bg-[#465940]/10 text-[#465940]' },
                      ...
                      { v: selected.omega3Mg,     label: 'Omega-3',                       unit: 'mg',   color: 'bg-[#465940]/10 text-[#465940]' },
                    ].filter(n => n.v);
                    if (!nutrients.length) return null;
                    return (
                      <div>
                        <p className="text-sm font-bold text-[#465940] mb-2">{t('კვებითი ღირებულება', 'Nutrition')}</p>
                  ...
                  {selected.allergens.length > 0 && (
                    <div>
                      <p className="text-sm font-bold text-[#465940] mb-2">{t('შეიცავს ალერგენებს', 'Contains allergens')}</p>
                ...
                <div className="rounded-2xl bg-[#465940] p-6 text-center">
                  <div className="w-12 h-12 bg-[#FDFBF0]/10 rounded-full flex items-center justify-center mx-auto mb-3">
                    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#FDFBF0" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                      <rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/>
                    </svg>
                  </div>
                  <p className="font-bold text-[#FDFBF0] mb-1">{t('სრული რეცეპტი დახურულია', 'Full recipe is locked')}</p>
                  <p className="text-sm text-[#FDFBF0]/70 mb-4">
                  ...
                  <a href={isLoggedIn ? '/#pricing' : '/signup'}
                    className="inline-block bg-[#FDFBF0] text-[#465940] font-bold text-sm px-6 py-3 rounded-full hover:opacity-90 transition">
