...
export default function EditMealForm({ dish }: { dish: any }) {
  const router = useRouter();
  const fileRef = useRef<HTMLInputElement>(null);

  const [titleKa, setTitleKa] = useState(dish.titleKa ?? '');
  const [titleEn, setTitleEn] = useState(dish.titleEn ?? '');
  const [descKa, setDescKa] = useState(dish.descriptionKa ?? '');
  const [descEn, setDescEn] = useState(dish.descriptionEn ?? '');
  const [ingredientsKa, setIngredientsKa] = useState((dish.ingredientsKa ?? []).join(', '));
  const [ingredientsEn, setIngredientsEn] = useState((dish.ingredientsEn ?? []).join(', '));
  const [mealType, setMealType] = useState(dish.mealType ?? 'BREAKFAST');
  const [calories, setCalories] = useState(dish.calories != null ? String(dish.calories) : '');
  ...
  NUTRIENTS.forEach(({ key }) => {
    if (dish[key] != null && Number(dish[key]) > 0) initNutrients[key] = String(dish[key]);
  });
      ...
      <form onSubmit={handleSubmit} className="space-y-5" noValidate>
        ...
        <div className={sec}>
          ...
          <div
            ...
            className={`relative cursor-pointer rounded-2xl border-2 border-dashed transition flex items-center justify-center overflow-hidden
              ${dragOver ? 'border-[#465940] bg-[#FDFBF0]' : 'border-[#465940]/20 hover:border-[#465940] hover:bg-[#465940]'}`}
            style={{ minHeight: imagePreview ? 240 : 160 }}
          >
            {imagePreview ? (
              <>
                <img src={imagePreview} alt="" className="w-full object-cover absolute inset-0" style={{ maxHeight: 280 }} />
          ...
          <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) setImage(f); }} />
        ...
        <div className={`${sec} space-y-4`}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><label className={lbl}>სახელი ქართულად *</label>
              <input required className={inp} value={titleKa} onChange={(e) => setTitleKa(e.target.value)} /></div>
            <div><label className={lbl}>სახელი ინგლისურად *</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><label className={lbl}>ინგრედიენტები ქართ. (მძიმით)</label>
              <textarea className={`${inp} min-h-[80px] resize-none`} value={ingredientsKa} onChange={(e) => setIngredientsKa(e.target.value)} /></div>
            <div><label className={lbl}>ინგრედიენტები ინგ. (comma)</label>
              <textarea className={`${inp} min-h-[80px] resize-none`} value={ingredientsEn} onChange={(e) => setIngredientsEn(e.target.value)} /></div>
          </div>
        </div>
        ...
        <div className={`${sec} space-y-5`}>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {AGE_GROUPS.map((ag) => (
                <label key={ag.value}
                  className={`flex items-center gap-2.5 px-4 py-3 rounded-xl border cursor-pointer transition ${ageGroups.includes(ag.value) ? 'border-[#465940] bg-[#FDFBF0]' : 'border-[#465940]/20 hover:border-[#465940]/30'}`}>
        ...
        <div className={`${sec} space-y-4`}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><label className={lbl}>კალორიები (kcal)</label>
              <input type="number" min="0" className={inp} value={calories} onChange={(e) => setCalories(e.target.value)} placeholder="0" /></div>
            <div><label className={lbl}>ცილა — Protein (g)</label>
              <input type="number" min="0" step="0.1" className={inp} value={protein} onChange={(e) => setProtein(e.target.value)} placeholder="0" /></div>
          </div>
