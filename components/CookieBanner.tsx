    ...
    <div className="fixed bottom-0 left-0 right-0 z-50 p-4 sm:p-6">
      <div className="max-w-3xl mx-auto bg-white rounded-2xl shadow-2xl border border-gray-100 p-5 flex flex-col sm:flex-row items-start sm:items-center gap-4">
        ...
        <div className="flex gap-2 shrink-0">
          <button
            onClick={decline}
            className="px-4 py-2 rounded-full text-xs font-bold text-gray-500 border border-gray-200 hover:bg-gray-50 transition"
          >
            {ka ? 'უარყოფა' : 'Decline'}
          </button>
          <button
            onClick={accept}
            className="px-4 py-2 rounded-full text-xs font-bold text-white bg-[#ff7f50] hover:bg-[#e86e40] transition"
          >
            {ka ? 'მიღება' : 'Accept'}
          </button>
        </div>
