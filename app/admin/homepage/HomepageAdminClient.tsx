...
function ImageField({ label, name, value, onChange }: {
  ...
  return (
    <div>
      <label className="block text-xs font-semibold text-[#465940]/70 mb-1">{label}</label>
      <div
      ...
      >
        {uploading ? (
          <div className="text-center py-6">
            ...
            <p className="text-xs text-[#465940]/70 font-semibold">იტვირთება...</p>
          </div>
        ) : value ? (
          <>
          ...
          <div className="text-center py-6 px-4">
            ...
            <p className="text-xs font-semibold text-[#465940]/80">ჩააგდე ფოტო ან დააჭირე</p>
            <p className="text-xs text-[#465940]/60 mt-0.5">JPG, PNG, WEBP</p>
          </div>
        )}
      </div>
      {value && (
        <input value={value} onChange={e => onChange(name, e.target.value)}
          placeholder="URL"
          className="mt-2 w-full border border-[#465940]/20 rounded-xl px-3 py-1.5 text-xs text-[#465940]/70 focus:outline-none focus:border-[#465940]"
...
function Field({ label, name, value, onChange, textarea }: {
  ...
  const cls = "w-full border border-[#465940]/20 rounded-xl px-3 py-2 text-sm text-[#465940] bg-white focus:outline-none focus:border-[#465940]";
  return (
    <div>
      <label className="block text-xs font-semibold text-[#465940]/70 mb-1">{label}</label>
      {textarea
  ...
  return (
    <div className="p-8 max-w-4xl">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-3xl font-black text-[#465940]">მთავარი გვერდი</h1>
          <p className="text-[#465940]/60 text-sm mt-1">ყველა ტექსტი და ფოტო მართეთ აქედან</p>
        </div>
        <button onClick={save} disabled={saving}
          className={`px-6 py-2.5 rounded-full font-bold text-sm transition disabled:opacity-50 ${saved ? 'bg-[#465940] text-[#FDFBF0]' : 'bg-[#465940] hover:bg-[#465940] text-[#FDFBF0]'}`}>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {TABS.map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-4 py-2 rounded-full text-sm font-bold transition ${tab === t ? 'bg-[#465940] text-[#FDFBF0]' : 'bg-[#FDFBF0] border border-[#465940]/20 text-[#465940]/70 hover:border-[#465940] hover:text-[#FDFBF0]'}`}>
            {TAB_LABELS[t]}
          </button>
        ))}
      </div>

      <div className="bg-[#FDFBF0] rounded-2xl border border-[#465940]/10 shadow-sm p-6 space-y-5">

        {/* HERO */}
        {tab === 'hero' && <>
          <h2 className="font-black text-[#465940] text-lg border-b pb-2">Hero სექცია</h2>
        ...
        {/* FEATURES */}
        {tab === 'features' && <>
          <h2 className="font-black text-[#465940] text-lg border-b pb-2">სარგებლების სექცია</h2>
          ...
          {[1,2,3].map(i => (
            <div key={i} className="border border-[#465940]/10 rounded-xl p-4 space-y-3">
              <p className="text-xs font-black text-[#465940]/60 uppercase">ბარათი {i}</p>
        ...
        {/* SAMPLES */}
        {tab === 'samples' && <>
          <h2 className="font-black text-[#465940] text-lg border-b pb-2">კვების ფოტოების სექცია</h2>
          <p className="text-xs text-[#465940]/60 bg-[#465940]/5 rounded-xl px-4 py-3">
        ...
        {/* PRICING */}
        {tab === 'pricing' && <>
          <h2 className="font-black text-[#465940] text-lg border-b pb-2">ფასების სექცია</h2>
