                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      ...
                      {canDisable ? (
                        isOff ? (
                          <span className="flex-shrink-0 text-xs bg-gray-200 text-gray-500 px-2 py-0.5 rounded-full font-bold">წაშლილი</span>
                        ) : (
                    ...
                    {isOff && (
                      <p className="text-xs text-amber-600 bg-amber-50 rounded-lg px-2.5 py-1.5 -mt-1">
                        ⚠️ წაშლილია — ეს მეილი არავის ეგზავნება, სანამ არ დააბრუნებთ.
                      </p>
                    )}
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex-1 border border-[#465940]/30 text-[#465940] py-2 rounded-xl text-sm font-bold hover:bg-[#465940]/5 transition"
                      >
                        ✏️ რედაქტირება
                      </button>
                      {canDisable && (
                        isOff ? (
                          <button
                            type="button"
                            onClick={() => toggleTemplateEnabled(t.key, true)}
                            disabled={togglingKey === t.key}
                            className="flex-1 border border-green-600 text-green-700 py-2 rounded-xl text-sm font-bold hover:bg-green-50 transition disabled:opacity-50"
                          >
                            {togglingKey === t.key ? '...' : '↩️ დაბრუნება'}
                          </button>
                        ) : (
                          <button
                            type="button"
                            onClick={() => {
                              if (window.confirm(`წავშალო "${meta?.name ?? t.key}"? ეს მეილი აღარავის გაეგზავნება, სანამ არ დააბრუნებთ. თავად შაბლონი (ტექსტი) დაცული რჩება — შეგეძლებათ ნებისმიერ დროს დაბრუნება.`)) {
                                toggleTemplateEnabled(t.key, false);
                              }
                            }}
                            disabled={togglingKey === t.key}
                            className="flex-1 border border-red-300 text-red-600 py-2 rounded-xl text-sm font-bold hover:bg-red-50 transition disabled:opacity-50"
                          >
                            {togglingKey === t.key ? '...' : '🗑️ წაშლა'}
                          </button>
                        )
                      )}
                    </div>
