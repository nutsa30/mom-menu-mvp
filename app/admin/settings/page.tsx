...
export default function AdminSettings() {
  ...
  return (
    <div className="p-8 max-w-2xl">
      <h1 className="text-3xl font-black text-gray-900 mb-1">{d.settingsTitle}</h1>
      <p className="text-gray-400 text-sm mb-8">{d.manageCredentials}</p>

      {/* Email change */}
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8 mb-6">
        <h2 className="text-lg font-bold text-gray-800 mb-1">ელ-ფოსტის შეცვლა</h2>
        <p className="text-sm text-gray-400 mb-6">შეიყვანე ახალი ელ-ფოსტა და დაადასტურე მიმდინარე პაროლით</p>
        ...
        <form onSubmit={handleEmailChange} className="space-y-4">
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1.5">ახალი ელ-ფოსტა</label>
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1.5">{d.currentPassword}</label>
          ...
          <button type="submit" disabled={emailStatus === 'loading'}
            className="bg-[#ff7f50] hover:bg-[#e86e40] text-white px-6 py-3 rounded-full font-bold text-sm transition disabled:opacity-60">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8">
        <h2 className="text-lg font-bold text-gray-800 mb-1">{d.changePassword}</h2>
        <p className="text-sm text-gray-400 mb-6">{d.updateAdminPassword}</p>
        ...
        <form onSubmit={handlePasswordChange} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1.5">{d.newPassword}</label>
            <input type="password" required minLength={6} value={pwNew} onChange={e => setPwNew(e.target.value)}
              className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:border-[#ff7f50] transition text-sm" />
          </div>
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1.5">{d.confirmPassword}</label>
            <input type="password" required value={pwConfirm} onChange={e => setPwConfirm(e.target.value)}
              className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:border-[#ff7f50] transition text-sm" />
          </div>

          {pwStatus === 'error' && (
            <p className="text-red-500 text-sm font-medium">{pwError}</p>
          )}
