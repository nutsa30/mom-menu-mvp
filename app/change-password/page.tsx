...
export default function ChangePassword({ searchParams }: { searchParams: { lang?: Locale } }) {
  ...
  return (
    <main className="min-h-screen bg-[#fff8f6] flex items-center justify-center px-6 py-12">
      <div className="w-full max-w-md bg-white rounded-[32px] shadow-xl p-10">
        {status === 'success' ? (
          <div className="text-center">
            ...
            <a href={`/dashboard?lang=${locale}`} className="text-[#ff7f50] font-bold hover:underline text-sm">
          ...
          <>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button
                ...
                className="w-full bg-[#ff7f50] text-white py-3.5 rounded-full font-bold text-sm shadow-md hover:bg-[#e86e40] transition disabled:opacity-60 mt-2"
              >
                {status === 'loading'
            ...
            <p className="text-sm text-gray-400 mt-6 text-center">
              <a href={`/dashboard?lang=${locale}`} className="text-[#ff7f50] font-bold hover:underline">
                ← {locale === 'ka' ? 'დეშბორდზე დაბრუნება' : 'Back to dashboard'}
              </a>
            </p>
          </>
