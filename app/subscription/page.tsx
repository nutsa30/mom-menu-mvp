    ...
    <main className="min-h-screen bg-[#6F7A5C] px-6 py-16">
      <div className="mx-auto max-w-4xl text-center">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          ...
          {/* RECIPE_PLAN */}
          <div className="rounded-[28px] bg-[#F5F1E4] p-8 flex flex-col min-w-0">
            <h2 className="text-xl font-semibold text-[#6F7A5C] mb-4">რეცეპტებზე წვდომა</h2>
            <div className="mb-6">
              {discountedPrice('RECIPE_PLAN', 15) ? (
                <div className="flex items-end gap-2 justify-center">
                  <span className="text-3xl line-through text-[#6F7A5C]/40 font-bold">15₾</span>
                  <span className="text-4xl font-black text-[#6F7A5C]">{discountedPrice('RECIPE_PLAN', 15)}₾</span>
                  <span className="text-[#6F7A5C]/60 text-sm">/თვე</span>
                </div>
              ) : (
                <div className="flex items-end gap-1 justify-center">
                  <span className="text-4xl font-black text-[#6F7A5C]">15₾</span>
                  <span className="text-[#6F7A5C]/60 text-sm mb-1">/თვე</span>
                </div>
              )}
            </div>
            ...
            {promoStatus['RECIPE_PLAN']?.msg && (
              <p className="text-[#6F7A5C] text-xs mb-2 font-semibold">{promoStatus['RECIPE_PLAN'].msg}</p>
            )}
            <button
              onClick={() => handleSubscribeBog('RECIPE_PLAN')}
              disabled={loadingPlan !== null || currentPlan === 'RECIPE_PLAN'}
              className="w-full py-3.5 mt-3 border border-[#6F7A5C] text-[#6F7A5C] rounded-full font-semibold hover:bg-[#6F7A5C]/10 transition disabled:opacity-60"
            >
          ...
          {/* FULL_PLAN */}
          <div className="rounded-[28px] bg-[#F5F1E4] p-8 flex flex-col min-w-0 relative">
            <div className="absolute -top-4 left-1/2 -translate-x-1/2">
              <span className="inline-flex items-center gap-2 text-[#F5F1E4] text-sm font-black px-6 py-2 rounded-full shadow-md" style={{ background: '#6F7A5C' }}>
                ⭐ საუკეთესო არჩევანი
              </span>
            </div>
            <h2 className="text-xl font-semibold text-[#6F7A5C] mb-2 mt-4">სრული პაკეტი</h2>
            <div className="mb-4">
              {discountedPrice('FULL_PLAN', 30) ? (
                <div className="flex items-end gap-2 justify-center">
                  <span className="text-3xl line-through text-[#6F7A5C]/40 font-bold">30₾</span>
                  <span className="text-4xl font-black text-[#6F7A5C]">{discountedPrice('FULL_PLAN', 30)}₾</span>
                  <span className="text-[#6F7A5C]/60 text-sm">/თვე</span>
                </div>
              ) : (
                <div className="flex items-end gap-2 justify-center">
                  <span className="text-3xl line-through text-[#6F7A5C]/40 font-bold">30₾</span>
                  <span className="text-4xl font-black text-[#6F7A5C]">21₾</span>
                  <span className="text-[#6F7A5C]/60 text-sm mb-1">/თვე</span>
                </div>
              )}
              ...
              <p className="text-[#6F7A5C]/60 text-sm mt-1">ყველაზე პოპულარული</p>
            </div>
            <ul className="space-y-3 text-left text-[#6F7A5C] flex-1 text-sm mb-6">
            ...
            <div className="flex gap-2 mb-1">
              ...
              <button
                ...
                className="px-4 py-2 text-[#F5F1E4] rounded-xl text-xs font-bold transition disabled:opacity-40"
            ...
            {promoStatus['FULL_PLAN']?.msg && (
              <p className="text-[#6F7A5C] text-xs mb-2 font-semibold">{promoStatus['FULL_PLAN'].msg}</p>
            )}
            <button
              onClick={() => handleSubscribeBog('FULL_PLAN')}
              disabled={loadingPlan !== null || currentPlan === 'FULL_PLAN'}
              className="w-full py-3.5 mt-3 text-[#F5F1E4] rounded-full font-bold shadow-lg transition disabled:opacity-60"
              style={{ background: '#6F7A5C' }}
            >
