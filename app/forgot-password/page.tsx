...
export default function ForgotPassword({ searchParams }: { searchParams: { lang?: Locale } }) {
  ...
  const handleReset = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        if (data.error === 'invalid_code') setError(locale === 'ka' ? 'კოდი არასწორია.' : 'Invalid code.');
            ...
            {step === 'email' && (
              <>
                ...
                <p className="text-[#465940]/60 text-sm mb-8">
                  {locale === 'ka'
                    ? 'შეიყვანეთ ელფოსტა — გამოგიგზავნით 6-ნიშნა კოდს.'
              ...
              <>
                <div className="mb-6">
                  ...
                  <p className="text-[#465940]/60 text-sm">
                    {locale === 'ka'
                      ? `შეამოწმეთ ${email} — გამოგიგზავნეთ 6-ნიშნა კოდი.`
                      : `Check ${email} — we sent you a 6-digit code.`}
                  </p>
                </div>
                ...
                <p className="text-sm text-[#465940]/60 mt-4 text-center">
                  ...
                  <button onClick={() => { setStep('email'); setDigits(['','','','','','']); setError(''); }}
