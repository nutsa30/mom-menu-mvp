...
export default function ForgotPassword({ searchParams }: { searchParams: { lang?: Locale } }) {
  ...
  return (
    <div className="min-h-screen bg-[#6F7A5C] flex items-center justify-center p-3 sm:p-6">
      <div className="w-full max-w-4xl bg-[#F5F1E4] rounded-3xl sm:rounded-[32px] shadow-2xl overflow-hidden grid lg:grid-cols-[45%_55%]" style={{ minHeight: 480 }}>

        {/* Left panel */}
        <div className="hidden lg:flex bg-[#6F7A5C] flex-col p-10 relative">
          <span className="text-[#F5F1E4] text-lg font-extrabold tracking-tight">Mom Menu</span>
          <div className="absolute top-[-80px] right-[-80px] w-64 h-64 rounded-full bg-[#F5F1E4]/10" />
          <div className="absolute bottom-[-60px] left-[-60px] w-80 h-80 rounded-full bg-[#F5F1E4]/10" />
          <div className="flex-1 flex items-center justify-center">
            <div className="w-full max-w-[260px]">
              <div className="bg-[#6F7A5C] rounded-3xl overflow-hidden" style={{ height: 280 }}>
        ...
        {/* Right panel */}
        <div className="flex items-center justify-center bg-[#F5F1E4] px-4 py-7 sm:px-10 sm:py-12">
          <div className="w-full max-w-sm">
            ...
            {step === 'success' && (
              <div className="text-center">
                ...
                <button onClick={() => router.push('/login?lang=' + locale)}
                  className="w-full bg-[#6F7A5C] text-[#F5F1E4] py-3.5 rounded-full font-bold text-sm shadow-md hover:opacity-90 transition">
            ...
            {step === 'email' && (
              <>
                <h1 className="text-2xl sm:text-3xl font-black text-[#6F7A5C] mb-1">
                ...
                <form onSubmit={handleSendCode} className="space-y-4">
                  <div>
                    ...
                    <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)}
                      className="w-full px-4 py-3 rounded-xl border border-[#6F7A5C]/20 focus:outline-none focus:border-[#6F7A5C] transition text-sm text-[#6F7A5C]" />
                  </div>
                  {error && <p className="text-red-600 text-sm">{error}</p>}
                  <button type="submit" disabled={loading}
                    className="w-full bg-[#6F7A5C] text-[#F5F1E4] py-3.5 rounded-full font-bold text-sm shadow-md hover:opacity-90 transition disabled:opacity-60">
            ...
            {step === 'code' && (
              <>
                <div className="mb-5">
                  ...
                  <h1 className="text-lg sm:text-2xl font-black text-[#6F7A5C] mb-1">
                    {locale === 'ka' ? 'კოდი გამოგზავნილია' : 'Code sent'}
                  </h1>
                  <p className="text-[#6F7A5C]/60 text-xs sm:text-sm break-all">
                ...
                <form onSubmit={handleVerifyCode} className="space-y-5">
                  ...
                  <button type="submit" disabled={loading || digits.join('').length < 6}
                    className="w-full bg-[#6F7A5C] text-[#F5F1E4] py-3 sm:py-3.5 rounded-full font-bold text-sm shadow-md hover:opacity-90 transition disabled:opacity-40">
                    {loading ? (locale === 'ka' ? 'მოწმდება...' : 'Verifying...') : (locale === 'ka' ? 'კოდის დადასტურება' : 'Verify code')}
                  </button>
                </form>
                <p className="text-sm text-[#6F7A5C]/60 mt-4 text-center">
                  {locale === 'ka' ? 'კოდი არ მიიღეთ? ' : "Didn't get the code? "}
                  <button onClick={() => { setStep('email'); setError(''); }}
                    className="text-[#6F7A5C] font-bold hover:underline">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input type={showPass ? 'text' : 'password'} required minLength={6}
                        ...
                        className="w-full px-4 py-3 rounded-xl border border-[#6F7A5C]/20 focus:outline-none focus:border-[#6F7A5C] transition text-sm pr-20 text-[#6F7A5C]" />
                      <button type="button" onClick={() => setShowPass(!showPass)}
                        className="absolute right-3 top-1/2 -translate-y-1/2 text-[#6F7A5C]/40 hover:text-[#6F7A5C] text-xs">
